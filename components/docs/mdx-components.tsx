...
export function createMdxComponents(): MDXComponents {
  ...
  return {
    h1: (props) => <h1 className="mt-10 text-3xl font-bold tracking-tight text-foreground" {...props} />,
    h2: (props) => {
      const id = generateHeadingId(typeof props.children === "string" ? props.children : undefined);
      return (
        <h2
          id={id}
    ...
    h3: (props) => {
      const id = generateHeadingId(typeof props.children === "string" ? props.children : undefined);
      return <h3 id={id} className="mt-8 scroll-mt-20 text-xl font-semibold text-foreground" {...props} />;
    },
    h4: (props) => {
      const id = generateHeadingId(typeof props.children === "string" ? props.children : undefined);
      return <h4 id={id} className="mt-6 scroll-mt-20 text-lg font-semibold text-foreground" {...props} />;
    },
    ...
    pre: (props) => (
      <div className="group relative mt-4">
        <CopyButton />
        <pre
          className="overflow-x-auto rounded-xl border border-border bg-[var(--code-bg)] p-4 text-sm leading-relaxed"
          {...props}
        />
      </div>
    ),
    code: (props) => {
      ...
      if (isInline) {
        return <code className="rounded-md bg-muted px-1.5 py-0.5 text-sm font-mono text-accent-green" {...props} />;
      }
      return <code {...props} />;
    },
    table: (props) => (
    ...
    th: (props) => (
      <th
        className="border-b border-border px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-foreground/80"
        {...props}
      />
    ),
