...
export default function PipValueCalculator() {
  ...
  const [currentPrice, setCurrentPrice] = useState<number>(1.09);
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-card border border-border rounded-2xl backdrop-blur-xl p-8">
        ...
        <div className="mt-8 space-y-6">
          {/* Currency Pair */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-2">Select Currency Pair</label>
            <select
              value={currencyPair}
          ...
          {/* Lot Size */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-2">Lot Size (Standard Lots)</label>
            <input
              type="number"
              value={lotSize}
          ...
          {/* Current Price */}
          <div>
            <label className="block text-sm font-medium text-foreground mb-2">Current Price (Optional)</label>
            <input
              type="number"
              value={currentPrice}
        ...
        <div className="mt-8 bg-muted border border-border rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-3xl font-bold text-green-400 mt-1">${pipValue.toFixed(2)}</p>
            </div>

            <div>
              <p className="text-muted-foreground text-sm">Per Point Value</p>
              <p className="text-3xl font-bold text-blue-400 mt-1">${pointValue.toFixed(2)}</p>
            </div>

            <div className="col-span-2 mt-2 p-3 bg-card/50 border border-blue-500/20 rounded-lg">
              <p className="text-sm text-foreground">
                <span className="font-semibold">Lot Size:</span> {lotSize} standard lot(s) ={" "}
                {(lotSize * 100000).toFixed(0)} units
              </p>
              <p className="text-sm text-foreground mt-1">
                <span className="font-semibold">1 Pip Move:</span> Price change of{" "}
                {pairInfo.decimals === 4 ? "0.0001" : "0.01"} = ${pipValue.toFixed(2)}
              </p>
            </div>
