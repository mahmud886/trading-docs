...
<Callout type="info" title="MSS in ICT Methodology">
  ICT (Inner Circle Trader) uses MSS to describe the moment when the algorithm shifts from one directional delivery to
  another. It represents the "shift" in the market-making algorithm's intent.
</Callout>
...
<Callout type="tip" title="The Perfect MSS Setup">
  When you see a liquidity sweep followed by displacement and MSS, you have the highest-probability reversal setup in
  SMC/ICT trading. The FVG or OB left behind during the MSS move is your entry zone.
</Callout>
