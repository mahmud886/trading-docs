...
dirs.forEach(dir => {
  ...
  files.forEach(fn => {
    ...
    lines.forEach((line, i) => {
      ...
      if (/\{[a-zA-Z]/.test(line) && line.indexOf('Callout') === -1 && line.indexOf('<') === -1 && line.indexOf('```') === -1) {
        // skip frontmatter
    ...
    if (dir === 'content/bn/academy') {
      ...
      if (fs.existsSync(enPath)) {
        ...
        if (bnBody === enBody && bnBody.length > 100) {
          issues.push({ file: filepath, line: 0, type: 'IDENTICAL_CONTENT', text: 'Bengali content is IDENTICAL to English - not translated' });
        }
      }
    }
  });
